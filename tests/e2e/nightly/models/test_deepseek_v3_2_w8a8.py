...
async def test_models(model: str, tp_size: int, dp_size: int) -> None:
    ...
    server_args = [
        ...
        "--compilation-config",
        '{"cudagraph_capture_sizes":[2, 4, 6, 8], "cudagraph_mode":"FULL_DECODE_ONLY"}',
        "--speculative-config", '{"num_speculative_tokens":2, "method":"mtp"}',
        "--tokenizer-mode", "deepseek_v32"
        # "--chat-template",
        # """{% if messages[0]['role'] == 'system' %}{% set system_message = messages[0]['content'] %}{% set messages = messages[1:] %}{% else %}{% set system_message = false %}{% endif %}{% for message in messages %}{% if message['role'] == 'user' %}{{ bos_token + '[INST] ' + message['content'] + ' [/INST]' }}{% elif message['role'] == 'assistant' %}{{ ' ' + message['content'] + eos_token }}{% endif %}{% endfor %}"""
    ]
